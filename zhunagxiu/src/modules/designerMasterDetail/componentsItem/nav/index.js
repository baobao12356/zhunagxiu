...
export default class Nav extends Component {
  ...
  handleShop() {
    ...
    if (shopId.indexOf('flagship') > 0) {
      ...
      location.href = `${Host.path}/flagshipShop?id=${shopId}&__open=1`;
    } else {
      location.href = `${Host.path}/shopDetail?id=${shopId}&__open=1`;
    }
  }
  ...
  render() {
    ...
    let { shareIcon, openId } = this.props;
    let divShow = false;
      ...
      <div className={className}>

        {
          Env.rsApp && <nav className="com-page-nav" ref="nav">
            ...
            {shareIcon && <Appoint openId={openId} id={this.props.designerId} cls="appoint" text="预约" pointAppoint={pointAppoint} f={this.props.f} />}
            {title && <div>{title}</div>}
          </nav>
        }
