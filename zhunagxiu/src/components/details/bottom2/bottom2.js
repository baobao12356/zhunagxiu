...
export default class Bottom2 extends Component {
  ...
  _onHybridBridge() {
    ...
    let parameter = {
      roleType: "1",
      targetOpenId: this.props.detailInfo.designerOpenId,
      tag: 9
    }
    ...
    return (
      <div className="bottom2">
        <div>
          ...
          <span className={"clickLike likeState"+this.state.likeState} onClick={this._onLike.bind(this)}>{this.state.likedCount}</span>
          <span className={"clickCollect collectState"+this.state.collectState} onClick={this._onCollect.bind(this)}>收藏</span>
        </div>
        <div className="clickAppoint" onClick={this._onHybridBridge.bind(this)}>立即预约</div>
      </div>
    )
  }
}
